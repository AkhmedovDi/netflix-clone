...
function Row({ title, movies }: Props) {
  ...
  return (
    <div className="h-40 space-y-0.5 md:space-y-2">
      ...
      <div className="group relative md:-ml-2s">
        <ChevronLeftIcon
           onClick={() => handleClick("left")}         
          className={`absolute top-0 bottom-0 left-2 z-40 m-auto w-9 h-9
          cursor-pointer opacity-0 transition hover:scale-125 group-hover:opacity-100 ${!isMoved && "hidden"}`}
        
        />
        <div
          ref={rowRef}
          className="flex items-center space-x-0.5 md:space-x-2 scrollbar-hide overflow-x-scroll 
         md:p-2 "
        >
          {movies.map((movie) => (
        ...
        <ChevronRightIcon
          ...
          className="absolute top-0 bottom-0 right-2 z-40 m-auto w-9 h-9
